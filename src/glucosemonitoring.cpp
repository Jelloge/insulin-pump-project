...
double GlucoseMonitoring::getLatestReading() const {
    if (!glucoseReadings.isEmpty()) {
        double latestReading = glucoseReadings.last().second;
...
double GlucoseMonitoring::getLatestGlucoseReading() const {
    if (!glucoseReadings.isEmpty()) {
        return glucoseReadings.last().second;
    } else {
        return -1.0; // or some error/sentinel value if no readings
    }
}
